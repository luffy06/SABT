...
tf.app.flags.DEFINE_float("decay_rate", 0.2, "Rate of decay for learning rate.") #0.65一次衰减多少
...
tf.app.flags.DEFINE_integer("num_epochs",2000,"number of epochs to run.")
        ...
        for epoch in range(curr_epoch,FLAGS.num_epochs):
            ...
            if epoch % FLAGS.validate_every==0:
                ...
                if best_valid_acc < eval_acc:
                    print('验证集准确度提高，保存模型！')
                    save_path=FLAGS.ckpt_dir+"model_anls.ckpt"
                    saver.save(sess,save_path)
                    best_valid_acc = eval_acc
        
        print('训练完毕，保存模型！')
        save_path=FLAGS.ckpt_dir+"model_anls_final.ckpt"
        saver.save(sess,save_path)
